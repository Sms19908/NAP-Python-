...
function editarTarea($task_id, $user_id, $title, $description, $due_date)
{
    global $pdo;
    try {
        $sql = "UPDATE tasks set user_id = :user_id, title = :title, description = :description, due_date = :due_date where task_id = :task_id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'user_id' => $user_id,
            'title' => $title,
...
if (isset($_SESSION['user_id'])) {
    ...
    switch ($method) {
        ...
        case 'POST':
            ...
            if (isset($input['title'], $input['description'], $input['due_date'])) {
                ...
                if ($task_id > 0) {
                    ...
                    echo json_encode(value: ["messsage" => "Tarea creada: ID:" . $task_id]);
                } else {
        ...
        case 'PUT':
            $input = getJsonInput();
            if (isset($input['user_id'],$input['title'], $input['description'], $input['due_date']) && $_GET['task_id']) {
                $editResult = editarTarea($_GET['task_id'], $input['user_id'],$input['title'], $input['description'], $input['due_date']);
                if ($editResult) {
                    http_response_code(201);
